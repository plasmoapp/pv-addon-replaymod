package su.plo.replayvoice.audio;
...
import su.plo.voice.mod.client.audio.AlUtil;
...
public final class AlLoopbackOutputDevice extends BaseAudioDevice implements AlAudioDevice, AlListenerDevice, HrtfAudioDevice, OutputDevice<AlSource> {
    ...
    public AlLoopbackOutputDevice(PlasmoVoiceClient client, @Nullable String name) {
        super(client, name);
    ...
    public void open(@NotNull AudioFormat format, @NotNull Params params) throws DeviceException {
        ...
        runInContext(() -> {
            synchronized (this) {
                ...
                System.out.println("Loopback render supported: " + SOFTLoopback.alcIsRenderFormatSupportedSOFT(
                        devicePointer,
                        ...
                        + " channels: " + SOFTLoopback.ALC_STEREO_SOFT
                        + " format: " + SOFTLoopback.ALC_SHORT_SOFT
                );
                ...
                IntBuffer attr = BufferUtils.createIntBuffer(10)
                        ...
                        .put(ALC_FREQUENCY).put((int) format.getSampleRate())
                        .put(0);
                ((Buffer) attr).flip();
                ...
                this.contextPointer = ALC11.alcCreateContext(this.devicePointer, attr);
    ...
    public void close() {
        ...
        runInContext(() -> {
            synchronized (this) {
                ...
                LOGGER.info("Loopback device " + name + " closed");
    ...
    public AlSource createSource(boolean stereo, @NotNull Params params) throws DeviceException {
        ...
        try {
            AlSource source = StreamAlLoopbackSource.create(this, client, stereo, numBuffers);
            sources.add(source);
            return source;
    ...
    private long openDevice(String deviceName) throws DeviceException {
        ...
        if (deviceName == null) {
            // default device
            l = SOFTLoopback.alcLoopbackOpenDeviceSOFT((ByteBuffer) null);
        } else {
            l = SOFTLoopback.alcLoopbackOpenDeviceSOFT(deviceName);
        }
